...
namespace Model.Dao.Farmer
{
    public class SaleOfferDAO
    {
        ...
        public IEnumerable<SALE_OFFER> getSaleOfferByIdUser(int FarmerId)
        {
            ...
            IQueryable<SALE_OFFER> model = db.SALE_OFFER.Where(x => x.FARM.Id_Farmer == FarmerId && x.Remain_SaleQuantity  > 0 && x.Is_Deleted == false).OrderBy(x => x.Date_SaleOffer);
            ...
            foreach (var item in model)
            {
                item.Number_Of_Orders = Convert.ToByte(db.SALE_OFFER.Where(x => x.Id_SaleOffer == item.Id_SaleOffer)
                                          ...
                                          .Where(po_pod_to => po_pod_to.TO.Id_StatusTrans == 10 || po_pod_to.TO.Id_StatusTrans == 11)
                                          .Count());
